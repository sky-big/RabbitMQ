...
-type(amqp_method_name() ::
       ...
       | 'tx.select' | 'tx.select_ok' | 'tx.commit' | 'tx.commit_ok'
       | 'tx.rollback' | 'tx.rollback_ok' | 'confirm.select' | 'confirm.select_ok' )).
-type(amqp_method() ::
       ...
       | {60, 111} | {60, 120} | {90, 10} | {90, 11} | {90, 20} | {90, 21}
       | {90, 30} | {90, 31} | {85, 10} | {85, 11} )).
...
-type(amqp_method_field_name() ::
       ( arguments | delivery_tag | redelivered | routing_key
       | ticket | exchange | if_unused | consumer_tag
       | nowait | ticket | queue | ticket
       | delivery_tag | destination | source | routing_key
       | nowait | arguments | routing_key | prefetch_size
       | message_count | queue | ticket | destination
       | source | routing_key | nowait | arguments
       | exchange | delivery_tag | requeue | if_empty
       | requeue | exchange | ticket | consumer_tag
       | queue | passive | durable | exclusive
       | auto_delete | nowait | arguments | multiple
       | requeue | reply_code | queue | message_count
       | consumer_count | multiple | ticket | queue
       | exchange | routing_key | ticket | nowait
       | arguments | message_count | ticket | queue
       | ticket | nowait | no_ack | message_count
       | routing_key | locale | ticket | challenge
       | if_unused | response | nowait | consumer_tag
       | channel_max | reply_text | frame_max | ticket
       | heartbeat | immediate | channel_max | frame_max
       | requeue | heartbeat | exchange | queue
       | virtual_host | capabilities | version_minor | insist
       | no_ack | known_hosts | redelivered | reply_code
       | reply_text | exchange | class_id | method_id
       | prefetch_count | global | routing_key | reason
       | type | nowait | out_of_band | queue
       | arguments | channel_id | no_local | passive
       | active | delivery_tag | exclusive | active
       | arguments | reply_code | nowait | reply_text
       | exchange | class_id | method_id | delivery_tag
       | consumer_tag | nowait | realm | client_properties
       | exclusive | passive | active | write
       | read | routing_key | exchange | mechanism
       | ticket | mandatory | consumer_tag | version_major
       | ticket | cluster_id | server_properties | mechanisms
       | response | locales | durable | auto_delete
       | internal | nowait )).
