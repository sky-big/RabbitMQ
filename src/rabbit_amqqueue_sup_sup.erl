...
%%----------------------------------------------------------------------------
%% 启动rabbit_amqqueue_sup_sup监督进程
start_link() ->
	supervisor2:start_link({local, ?SERVER}, ?MODULE, []).


%% 在rabbit_amqqueue_sup_sup监督进程下启动一个rabbit_amqqueue_sup监督进程，同时在rabbit_amqqueue_sup监督进程下启动一个队列进程
...
%% rabbit_amqqueue_sup_sup监督进程启动的回调初始化
init([]) ->
