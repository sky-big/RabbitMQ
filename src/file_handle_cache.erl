...
%% 主要作用是管理每个进程拥有的文件描述符数量，确保每个进程不会超过设定的上限
...
-record(handle,
		{ hdl,							%% prim_file打开文件返回的文件句柄
		  ...
		  read_buffer_rem,        		%% Num of bytes from pos to end(保存的是num个字节从pos到文件结束)(上次读取的字符串数量 - read_buffer_pos)
		  read_buffer_size,       		%% Next size of read buffer to use(下一个要读取的缓存大小)
		  read_buffer_size_limit, 		%% Max size of read buffer to use(最大读取缓存的大小)
...
%% obtain/0方法增加拥有的文件描述符数量，transfer/1将文件描述符的拥有权转移到另外一个进程（原进程拥有的打开描述符数量减1，转移目标进程拥有的文件描述符数量加1）
obtain()      -> obtain(1).
release()     -> release(1).
