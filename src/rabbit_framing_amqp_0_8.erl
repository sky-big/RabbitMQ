...
-type(amqp_method_field_name() ::
       ( global | nowait | arguments | identifier
       | exchange | reply_code | queue | message_count
       | mandatory | consumer_count | reply_code | exclusive
       | consumer_tag | integer_2 | delivery_tag | cluster_id
       | ticket | queue | exchange | immediate
       | routing_key | consumer_tag | nowait | arguments
       | ticket | ticket | prefetch_size | redelivered
       | prefetch_count | consumer_tag | ticket | queue
       | nowait | consumer_tag | message_count | routing_key
       | queue | consumer_tag | delivery_tag | ticket
       | nowait | queue | delivery_tag | if_unused
       | if_empty | consumer_tag | nowait | multiple
       | consumer_tag | reply_text | message_count | no_local
       | content_size | immediate | version_major | nowait
       | nowait | version_minor | integer_3 | queue
       | prefetch_size | mechanisms | locales | no_ack
       | routing_key | arguments | identifier | client_properties
       | routing_key | consumer_tag | mechanism | response
       | queue | locale | exclusive | dtx_identifier
       | challenge | exchange | meta_data | identifier
       | response | prefetch_count | global | channel_max
       | string_1 | frame_max | nowait | heartbeat
       | routing_key | exchange | channel_max | frame_max
       | requeue | heartbeat | reply_text | consumer_tag
       | prefetch_size | delivery_tag | virtual_host | capabilities
       | queue | prefetch_count | insist | delivery_tag
       | nowait | exchange | known_hosts | ticket
       | integer_op | host | result | known_hosts
       | integer_4 | delivery_tag | consumer_tag | reply_code
       | reply_text | queue | class_id | ticket
       | method_id | ticket | consumer_tag | exchange
       | mandatory | consumer_tag | delivery_tag | out_of_band
       | no_ack | exchange | routing_key | delivery_tag
       | active | immediate | ticket | nowait
       | active | string_2 | integer_result | reply_code
       | reply_text | table | details | routing_key
       | queue | reply_code | reply_text | consumer_tag
       | class_id | result | method_id | mandatory
       | consume_rate | redelivered | exchange | requeue
       | routing_key | realm | ticket | exclusive
       | passive | requeue | string_op | active
       | operation | write | no_local | read
       | multiple | server_properties | ticket | redelivered
       | exchange | routing_key | content_checksum | ticket
       | no_ack | exchange | integer_1 | type
       | exchange | passive | durable | auto_delete
       | internal | exclusive | nowait | operation
       | arguments | routing_key | consumer_tag | reply_code
       | ticket | exchange | exchange | if_unused
       | nowait | message_count | requeue | reply_text
       | global | string_result | ticket | ticket
       | routing_key | queue | passive | no_local
       | staged_size | durable | exclusive | exchange
       | consumer_tag | auto_delete )).
