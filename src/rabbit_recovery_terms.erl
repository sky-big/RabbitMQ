...
%% 启动rabbit_recovery_terms进程(该进程启动在rabbit_sup监督进程下)
start() -> rabbit_sup:start_child(?MODULE).


%% 停止rabbit_recovery_terms进程
...
%%----------------------------------------------------------------------------
%% rabbit_recovery_terms进程的回调初始化函数
init(_) ->
...
%%----------------------------------------------------------------------------
%% 在mnesia数据库的存储目录下打开recovery.dets文件
open_table() ->
